...
```{r}
grafico = log_coef %>% 
  ggplot(aes(x = term, y = OR, ymin = OR.conf.low,
             ymax = OR.conf.high, color = p.value.level)) +
  geom_pointrange() +
  scale_x_discrete(labels = str_sub(log_coef$term,end = 10)) +
  scale_color_brewer(palette = "Spectral", direction = -1) +
  xlab("Coefficienti") +
  theme(legend.position = "none",
        axis.text.x = element_text(vjust = 1, size = 8, angle = 90)) 
